...
class CompanyImporter(boltUri: String, username: String? = null, password: String? = null) {
    ...
    private fun streamRows(reader: BufferedReader): Stream<Map<String, String>> {
        return reader.lines().skip(1).map {
            val fields = it.split(",")
            mapOf(
                    Pair("company_id", fields[0].toUpperCase(locale)),
                    Pair("country_code", fields[1].toUpperCase(locale)),
                    Pair("country_name", fields[2].toUpperCase(locale)),
                    Pair("sector_code", fields[3].toUpperCase(locale)),
                    Pair("sector_label", fields[4].toUpperCase(locale)),
                    Pair("company_name", fields[5].toUpperCase(locale)),
                    Pair("address", mergeAddress(
                            fields[6],
                            fields[7],
                            fields[8],
                            fields[9])),
                    Pair("zipcode", fields[10].toUpperCase(locale)),
                    Pair("city_name", fields[11].toUpperCase(locale))
            )
        }
    }

    private fun upsertCompanyGraph(session: Session, rows: List<Map<String, String>>): StatementResult? {
        return session.run("UNWIND {rows} AS row " +
                ...
                "RETURN true", mapOf(Pair("rows", rows)))
    }
    ...
    private fun mergeAddress(addressFirstPart: String, addressSecondPart: String, addressThirdPart: String, addressFourthPart: String): String {
        ...
        builder = builder.append(addressFirstPart.toUpperCase(locale))
